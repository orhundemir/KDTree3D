...
public class TreeMap<K, V> extends AbstractSortedMap<K, V> {
    ...
    protected static class BalanceableBinaryTree<K, V> extends LinkedBinaryTree<Entry<K, V>> {
        ...
        public Position<Entry<K, V>> rotateR(Position<Entry<K, V>> p) {
            Node<Entry<K, V>> z = (TreeMap.BalanceableBinaryTree.BSTNode<Entry<K, V>>) validate(p); //a
            Node<Entry<K, V>> y = (TreeMap.BalanceableBinaryTree.BSTNode<Entry<K, V>>) z.getRight();//b
            Node<Entry<K, V>> x = (TreeMap.BalanceableBinaryTree.BSTNode<Entry<K, V>>) z.getParent();//t2
        ...
        public Position<Entry<K, V>> rotateL(Position<Entry<K, V>> p){
            Node<Entry<K, V>> z = (TreeMap.BalanceableBinaryTree.BSTNode<Entry<K, V>>) validate(p); //a
            Node<Entry<K, V>> y = (TreeMap.BalanceableBinaryTree.BSTNode<Entry<K, V>>) z.getLeft();//b
            Node<Entry<K, V>> x = (TreeMap.BalanceableBinaryTree.BSTNode<Entry<K, V>>) z.getParent();//t2
        ...
        public void rotateLR(Position<Entry<K, V>> p){
            
        }

        public void rotateRL(Position<Entry<K, V>> p){
    ...
    protected void rotateL(Position<Entry<K, V>> p) {
        tree.rotate(p);
    }
    protected void rotateR(Position<Entry<K, V>> p) {
        tree.rotate(p);
    }
    protected void rotateLR(Position<Entry<K, V>> p) {
        tree.rotate(p);
    }
    protected void rotateRL(Position<Entry<K, V>> p) {
        tree.rotate(p);
    }

    public void print(){
        for(Position<Entry<K,V>> p:this.tree.preorder()){
            TreeMap.BalanceableBinaryTree.BSTNode<Entry<K, V>> x =(TreeMap.BalanceableBinaryTree.BSTNode<Entry<K, V>>) tree.validate(p);
            if(p.getElement()!=null)
            System.out.println(x.aux+": "+ x.getElement()+": "+tree.depth(p));
        }
    }
    ...
    public V put(K key, V value) throws IllegalArgumentException {
        ...
        if (isExternal(p)) {                    // key is new
            ...
            BalanceableBinaryTree.BSTNode<Entry<K,V>> cur=(TreeMap.BalanceableBinaryTree.BSTNode<Entry<K, V>>) tree.validate(p);
            if(isExternal(sibling(p))){
                while(p!=root()) {
                    cur.aux=cur.aux+1;
                    cur=(TreeMap.BalanceableBinaryTree.BSTNode<Entry<K, V>>) cur.getParent();
                    p=parent(p);
                }
                cur.aux=cur.aux+1;
            }
            else{
                //BalanceableBinaryTree.BSTNode<Entry<K,V>> sib=(TreeMap.BalanceableBinaryTree.BSTNode<Entry<K, V>>) tree.validate(sibling(p));
                cur.aux=0;
                //cur.setAux(sib.getAux());
            }
  ...
  public V remove(K key) throws IllegalArgumentException {
        ...
        Position<Entry<K,V>> p2 = p;
        BalanceableBinaryTree.BSTNode<Entry<K,V>> cur=(TreeMap.BalanceableBinaryTree.BSTNode<Entry<K, V>>) tree.validate(p2);
        if(p==root()) cur.aux=cur.aux+1;
        ...
        if(isExternal(sibling(p2)) && cur.aux==0){
            while(p2!=root()) {
                cur.aux=cur.aux-1;
                p2=parent(p2);
                cur=(TreeMap.BalanceableBinaryTree.BSTNode<Entry<K, V>>) tree.validate(p2);
            }
            cur.aux=cur.aux-1;
        }
        else if(isExternal(right(p2)) && cur.aux!=0){
            while(p2!=root()) {
                cur.aux=cur.aux-1;
                p2=parent(p2);
                cur=(TreeMap.BalanceableBinaryTree.BSTNode<Entry<K, V>>) tree.validate(p2);
    ...
    public V remove(K key) throws IllegalArgumentException {
        ...
        if (isExternal(p)) {                    // key not found
          ...
          else if(isExternal(left(p)) && isExternal(right(p)) ){
            if(isExternal(sibling(p))){
                while(cur!=root().getElement()) {
                    //cur.aux=cur.aux-1;
                    cur=(TreeMap.BalanceableBinaryTree.BSTNode<Entry<K, V>>) cur.getParent();
                }
                //cur.aux=cur.aux-1;
    ...
    */
    ...
    public int countLessThan(K k) {
        return countLessThanAux(k,root());
    }
    private int countLessThanAux(K key, Position<Entry<K, V>> p) {
        if (isInternal(p)) {
            if (compare(p.getElement(), key) <= 0){
                int count = 1+countLessThanAux(key, left(p));
                return count+countLessThanAux(key, right(p));
            }
            else{
                return countLessThanAux(key, left(p));
            }
        }
        return 0;
    }

    public K inOrderPredecessor(K key) throws IllegalArgumentException {
        checkKey(key);                              // may throw IllegalArgumentException
        Position<Entry<K, V>> p = treeSearch(root(), key);
        if (isInternal(p)) { //key bts de varsa
            if(p==treeMin(root())) return treeMax(root()).getElement().getKey(); //(ilk visit edilen nodun nodeun presuccessoru son visit edilen olmasini sagliyor)
            if(left(left(p))!=null) { //sol cocukta key varsa  sol subtreenin en sagi presuccessor
                p=left(p);
                while (isInternal(p)){
                    p=right(p);
                }
                return parent(p).getElement().getKey();
            }
        }
        while (!isRoot(p)) { //key bts de yoksa
            if (p == right(parent(p))) {
                return parent(p).getElement().getKey();          
            } else {
                p = parent(p);
                //if (compare(p.getElement(), key)>0) 
                    //return treeMax(root()).getElement().getKey(); //(ilk visit edilen nodun nodeun presuccessoru son visit edilen olmasini sagliyor)
            }
        }
        return null;                                
    }

    public K inOrderSuccessor(K key) throws IllegalArgumentException {
        checkKey(key);                              // may throw IllegalArgumentException
        Position<Entry<K, V>> p = treeSearch(root(), key);
        if (isInternal(p)) { //key bts de varsa
            if(p==treeMax(root())) return treeMin(root()).getElement().getKey(); //(son visit edilen nodun nodeun successoru ilk visit edilen olmasini sagliyor)
            if(right(right(p))!=null) { //sağ cocukta key varsa sag subtrenin en solu successor
                p=right(p);
                while (isInternal(p)){
                    p=left(p);
                }
        ...
        while (!isRoot(p)) { //key bts de yoksa
            if (p == left(parent(p))) {
                ...
                p = parent(p);
                //if (compare(p.getElement(), key)<0) 
                    //return treeMin(root()).getElement().getKey(); //(son visit edilen nodun nodeun presuccessoru ilk visit edilen olmasini sagliyor)
            }
        }
        return null;                                
    }
